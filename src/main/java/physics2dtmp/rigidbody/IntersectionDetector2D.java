...
public class IntersectionDetector2D {
    ...
    public static boolean pointOnLine(Vector2f point, Line2D line) {
        ...
        if (dx == 0f) {
            return MathUtils.compare(point.x, line.getStart().x);
        }
        float m = dy / dx;
    ...
    public static boolean pointInBox2D(Vector2f point, Box2D box) {
        ...
        MathUtils.rotate(pointLocalBoxSpace, box.getRigidbody().getRotation(),
    ...
    public static boolean lineAndBox2D(Line2D line, Box2D box) {
        ...
        MathUtils.rotate(localStart, theta, center);
        MathUtils.rotate(localEnd, theta, center);
    ...
    public static boolean raycast(Box2D box, Ray2D ray, RaycastResult result) {
        ...
        MathUtils.rotate(xAxis, -box.getRigidbody().getRotation(), new Vector2f(0, 0));
        MathUtils.rotate(yAxis, -box.getRigidbody().getRotation(), new Vector2f(0, 0));
        ...
        float[] tArr = {0, 0, 0, 0};
        for (int i=0; i < 2; i++) {
            if (MathUtils.compare(f.get(i), 0)) {
    ...
    public static boolean circleAndBox2D(Circle circle, Box2D box) {
        ...
        MathUtils.rotate(r, -box.getRigidbody().getRotation(), new Vector2f());
    ...
    public static boolean AABBAndBox2D(AABB b1, Box2D b2) {
        ...
        MathUtils.rotate(axesToTest[2], b2.getRigidbody().getRotation(), new Vector2f());
        MathUtils.rotate(axesToTest[3], b2.getRigidbody().getRotation(), new Vector2f());
        for (int i=0; i < axesToTest.length; i++) {
            if (!overlapOnAxis(b1, b2, axesToTest[i])) {
                return false;
            }
